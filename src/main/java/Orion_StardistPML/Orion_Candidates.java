...
public class Orion_Candidates {
    
    private final List<Path> polygons = new ArrayList<>();
    private final List<Orion_Point2D> origins = new ArrayList<>();
    private final List<Orion_Box2D> bboxes = new ArrayList<>();
    ...
    public Orion_Candidates(RandomAccessibleInterval<FloatType> prob, RandomAccessibleInterval<FloatType> dist) {
        this(prob, dist, 0.4);
    }

    public Orion_Candidates(RandomAccessibleInterval<FloatType> prob, RandomAccessibleInterval<FloatType> dist, double threshold) {
        this(prob, dist, threshold, 2, null);
    }

    public Orion_Candidates(RandomAccessibleInterval<FloatType> prob, RandomAccessibleInterval<FloatType> dist, double threshold, int b, LogService log) {        
        ...
        final double[] phis = Orion_Utils.rayAngles(nrays);
        ...
        for (int i = b; i < shape[0]-b; i++) {
            for (int j = b; j < shape[1]-b; j++) {
                ...
                if (score > threshold) {
                    ...
                    polygons.add(poly);
                    bboxes.add(new Orion_Box2D(xmin,xmax,ymin,ymax));
                    origins.add(new Orion_Point2D(S*i,S*j));
                    scores.add(score);
                    areas.add(poly.area());
                }
            }
        }
        score_indices = Orion_Utils.argsortDescending(scores);
    ...
    public void nms_v0(final double threshold) {
        ...
        for (int ii = 0; ii < n; ii++) {
            ...
            winner.add(i);
            final Orion_Box2D bbox = bboxes.get(i);
...
//    public Orion_Point2D getOrigin(int i) {
...
//    public Orion_Box2D getBbox(int i) {
//        return bboxes.get(i);
//    }
...
//    }
    ...
    public PolygonRoi getPolygonRoi(int i) {
        return Orion_Utils.toPolygonRoi(polygons.get(i), S);
    }
    
    public PointRoi getOriginRoi(int i) {
        return Orion_Utils.toPointRoi(origins.get(i), S);
    }
    
    public Roi getBboxRoi(int i) {
        return Orion_Utils.toBoxRoi(bboxes.get(i), S);
    }
