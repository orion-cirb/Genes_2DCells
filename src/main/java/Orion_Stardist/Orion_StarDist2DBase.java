...
public abstract class Orion_StarDist2DBase {
    ...
    protected URL getResource(final String name) {
        return Orion_StarDist2DBase.class.getClassLoader().getResource(name);
    }
    ...
    protected void export(String outputType, Orion_Candidates polygons, int framePosition, long numFrames, String roiPosition) {
        switch (outputType) {
        case Orion_Opt.OUTPUT_ROI_MANAGER:
            exportROIs(polygons, framePosition, numFrames, roiPosition);
            break;
        case Orion_Opt.OUTPUT_LABEL_IMAGE:
            exportLabelImage(polygons, framePosition);
            break;
        case Orion_Opt.OUTPUT_BOTH:
            exportROIs(polygons, framePosition, numFrames, roiPosition);
            exportLabelImage(polygons, framePosition);
            break;
        case Orion_Opt.OUTPUT_POLYGONS:
            exportPolygons(polygons);
            break;
        default:
            showError(String.format("Invalid %s \"%s\"", Orion_Opt.OUTPUT_TYPE, outputType));
        }
    }

    protected void exportROIs(Orion_Candidates polygons, int framePosition, long numFrames, String roiPosition) {
    ...
    protected void setRoiPosition(Roi roi, int framePosition, String roiPosition) {
        switch (roiPosition) {
        case Orion_Opt.ROI_POSITION_STACK:
            roi.setPosition(framePosition);
            break;
        case Orion_Opt.ROI_POSITION_HYPERSTACK:
            roi.setPosition(0, 0, framePosition);
            break;
        default:
            showError(String.format("Invalid %s \"%s\"", Orion_Opt.ROI_POSITION, roiPosition));
        }
    }

    protected void exportLabelImage(Orion_Candidates polygons, int framePosition) {
        if (labelImage == null)
    ...
    abstract protected void exportPolygons(Orion_Candidates polygons);

    abstract protected ImagePlus createLabelImage();

    protected Dataset labelImageToDataset(String outputType) {
        if (outputType.equals(Orion_Opt.OUTPUT_LABEL_IMAGE) || outputType.equals(Orion_Opt.OUTPUT_BOTH)) {
            if (labelCount > MAX_LABEL_ID) {
                log.error(String.format("Found more than %d segments -> label image does contain some repetitive IDs.\n(\"%s\" output instead does not have this problem).", MAX_LABEL_ID, Orion_Opt.OUTPUT_ROI_MANAGER));
            }
            ...
            final Dataset ds = Orion_Utils.raiToDataset(dataset, Orion_Opt.LABEL_IMAGE, labelImg, axes);
            // set LUT 
            try {
